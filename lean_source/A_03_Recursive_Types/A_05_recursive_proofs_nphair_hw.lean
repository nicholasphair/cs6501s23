...
-- Proof that appending is associative.
theorem list_append_assoc {α : Type u}:
  ∀ (a b c : list α),
...
-- base lemma
simp [list.append], -- induction on a lets me lineup the definition
simp [list.append],

assumption,
end

-- Slightly tweak the proof that nil is a right identity to generalize across any type universe u.
def nil_right_ident_append_nphair { α : Type u} : 
  ∀ (l : list α), l ++ [] = l :=
begin
assume l,
induction l,
...
-- Proof that nil is the identity elment for appending.
-- Shows that it is a left and right identity.
theorem nil_identity_append_list_nphair {α : Type u}: 
  ∀ (l : list α), 
    list.append list.nil l = l ∧ list.append l list.nil = l := 
    begin
    assume a,
    split,
    simp [list.append],
    apply nil_right_ident_append_nphair,
    end

-- Buld the monoid.
def monoid_list_append_nphair {α : Type}: @monoid (list α) :=
  monoid.mk list.append list.nil nil_identity_append_list_nphair list_append_assoc

#reduce foldr' monoid_list_append_nphair [[], [1, 2, 3]]
#reduce foldr' monoid_list_append_nphair [[0, 1, 2], [3, 4, 5]]
#reduce foldr' monoid_list_append_nphair [[], [0, 1, 2], [3, 4, 5], [6, 7, 8], []]
