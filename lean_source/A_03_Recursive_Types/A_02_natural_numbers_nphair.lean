...
| nat.zero m := m
| (nat.succ n') m := nat.succ (plus n' m ) -- in this case we've defined the non-zero cases as the successor of some value, n'
-- | (1 + n') m := 1 + (n' + m)  -- this is our addition. This rewrite gives us a recursive base case.
-- apply nat.succ to the second argument, the first argument number of times
...
-- n - m
-- let n=7, m = 5
-- 7 - 5 == 6 - 4 == 5 - 3 == 4 - 2 == 3 - 1 == 2 - 0 == 0
def less : nat → nat → nat
| 0 _ := 0
| n nat.zero := n
| (n' +1) (m' + 1) := less n' m'
...
-- add is iterative increment. mult is iterative add. exponentiation is iterative mult
