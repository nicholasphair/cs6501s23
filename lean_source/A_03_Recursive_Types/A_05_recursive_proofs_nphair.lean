...
-- simp looks at the rules in the list (eg., nat.add), and tries to simply the goal apply the rules.
-- QUOTE. 
...
rw ih, -- use the induction hypthesis to rewrite the goal.
end
-- QUOTE.
...
show 1 + (0 + 2) = 3, -- np: we are not blindly rewriting. We are doing strategically so we can apply ih
unfold P at ih,
end 
...
begin
assume a' ih, -- frm any a' we can get a proof for a' +1
unfold P at ih,
unfold P,
simp [nat.add],
-- same problem again
...
| 0 := p0
| (nat.succ a') := (step a' (pa a'))  -- step function on a' (which is really a - 1)
...
For our particular definition of *P a* at least, we've thus proved this::
  ...
  *∀ (a : ℕ),   -- for any nat a 
    P 0 →       -- if i have a proof of P 0 (some prop, P applied to 0)
    (∀ (a' : ℕ), P a' → P (a' + 1)) →  -- then if (for any nat a', and a proof of P for a' then I have a proof of a'+1)
    P a*   -- I cna prove any a* -- tis is te induction axiom for natural numbers
...
begin
assume a,
apply nat.rec_on a,   -- we left out the two proofs, our goal is to now provide them
exact rfl,    -- base case
...
begin
assume a,
induction a,  -- this gives us the induction hypot to use.
exact rfl,    -- base case
unfold P,
unfold P at a_ih,
simp [nat.add],
