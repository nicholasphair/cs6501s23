...
-- MONOID is a typclas. Im defining an instance of it so I can use rot_sym_eqtri in things like fold. 
...
-- rot_mull will "mul" in the has_mat structure (typeclass)
...
⟨
  rot_mul,
  rot_mul_assoc,
  1,  --- ohh we went through the effort of creating all of these intermediate insances, so we can get this 1 notation
      -- I can use 1 to refer to the identity element of any element! Because of has_one.
      -- Id get it later from monoid if i didnt do it when I did becasue monoid extends all those typeclasses
  rot_left_ident,
  rot_right_ident,
  rot_npow,
⟩ 
...
-- I get all of the opeations on monoids (e.g., fold)
-- I get this nice syntax from each typeclass
-- Id assume there is a mapping that given a 1, find the typelcass instance of has_mul  and pulls out the identity in that struture

-- a super critical tool chain.