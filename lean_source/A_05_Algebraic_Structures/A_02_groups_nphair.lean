...
div_inv_monoid.mk :
  Π -- arguments
    ...
    (one : G)
    (one_mul : ∀ (a : G), 1 * a = a)  -- this 1 not an int 1, its syntax for the `one` above
    (mul_one : ∀ (a : G), a * 1 = a)  -- similarly this is not a int multiplication it is whatever we deined multiplcation to be (e.g., rotation)
    (npow : ℕ → G → G)
...
inductive int : Type
| of_nat : nat → int
| neg_succ_of_nat : nat → int  -- to build -7 I do int.neg_succ_of_nat 6
                               -- succ_of_6 is 7, neg_7 is -7
-/
-- QUOTE. 
...
#reduce rot_zpow (-3: ℤ) r120  -- = 1/(r120 ^ 3) == 1 / (r120 * r120 * r120) = 1 / r360 = 1 / r0 = r0/r0 = r0 -- remember 1 is identity element. And we must have defined division for this to work. 
#reduce rot_zpow (-2: ℤ) r240  
-- QUOTE.
