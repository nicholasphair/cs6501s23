...
example: ∀ P Q R, P ∨ (Q ∧ R) → (P ∨ Q) ∧ (P ∨ R) :=
  ...
  assume P Q R,
  assume h,
  apply and.intro _ _, -- apply and.intro but we don't have proofs of left and right yet
  -- at this point i need to prove P or prove (Q ∧ R) 
  cases h with p qandr,
  apply or.inl p, -- inl vs intro_left
  cases qandr with q r,
  apply or.inr q,
  cases h with p qandr,
  exact or.inl p,
  cases qandr,
  exact or.inr qandr_right,
  end
