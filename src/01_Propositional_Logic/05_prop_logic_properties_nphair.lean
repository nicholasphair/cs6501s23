...
def not_involutes : 
  ∀ (e1 : prop_expr) 
    (i : prop_var → bool),
    (pEval (¬¬e1) i) = (pEval e1 i) :=
begin
assume e1 i,
unfold pEval,
unfold un_op_sem, 
cases (pEval e1 i),  -- expand "bnot (bnot (pEval e1 i)) = pEval e1 i" into the different individual cases
                     -- bnot (bnot (pEval e1 i)) can only be tt or tf so those are our two cases
                     -- pEval e1 i has to be tt or tf also for this to be true.
