...
theorem and_elimination_left:
  ∀ (e1 e2 : prop_expr) 
    (i : prop_var → bool),
    (pEval (e1 ∧ e1) i) = tt → (pEval e1 i) = tt :=
    begin
      assume e1 e2 i,
...
theorem negation_elimination:
  ∀ (e1 : prop_expr) (i : prop_var → bool),
    (pEval (¬¬e1) i) = tt → (pEval e1 i) = tt :=
    begin
      assume e1 i,
      unfold pEval,
      unfold un_op_sem,
      cases (pEval e1 i),
      unfold bnot,
      simp,     -- at this point I am left with proving false → false = true
      assume h, -- Not quite sure why I can just assume.
      assumption,
      unfold bnot,
      assume h,
      assumption,
...
-- tried to do without assume h.
theorem or_intoduction_left:
  ∀ (e1 e2: prop_expr) (i : prop_var → bool),
    (pEval e1 i) = tt → (pEval (e1 ∨ e2) i) = tt :=
    begin
      assume e1 e2 i,
      unfold pEval,
      unfold bin_op_sem,
...
theorem iff_introduction:
  ∀ (e1 e2: prop_expr) (i : prop_var → bool),
    ...
    begin
      ...
      simp,
      unfold bimp,
      simp, -- i'm left with proving true → false  = true which I cannot do.
      sorry,
